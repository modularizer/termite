...
def highlight_python(lines: list[str]) -> list[str]:
    """Python syntax highlighting."""
    styled_lines = []
    
    # Keywords
    keywords = r'\b(async|await|function|object|def|class|if|elif|else|for|while|try|except|finally|with|import|from|as|return|yield|pass|break|continue|lambda|and|or|not|in|is|None|True|False)\b'
    # Types and built-ins
    types = r'\b(int|str|float|bool|list|dict|tuple|set|frozenset|bytes|bytearray|object|type|NoneType)\b'
    self = r'\b(self)\b'
    # String literals
    strings = r'("(?:[^"\\]|\\.)*"|\'(?:[^\'\\]|\\.)*\')'
    # Numbers
